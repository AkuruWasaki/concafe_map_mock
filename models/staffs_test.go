...
func testStaffToOneShopUsingShop(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadShop(ctx, tx, false, (*[]*Staff)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Shop == nil {
...
func testStaffToOneSetOpShopUsingShop(t *testing.T) {
	var err error
...
var (
	staffDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`, `Age`: `int`, `ShopID`: `int`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_            = bytes.MinRead
)
